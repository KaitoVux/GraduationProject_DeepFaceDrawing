...
class CombineModel():
    def __init__(self):
        ...
        if self.inmodel:
            ...
            for key in self.model.keys():
                ...
                self.mask[key] = cv2.cvtColor(cv2.imread(
                    'heat/' + key + '.jpg'), cv2.COLOR_RGB2GRAY).astype(np.float) / 255
        ...
        self.random_ = random.randint(
            0, self.model[''].feature_list[self.sex].shape[0])
    ...
    def predict_shadow(self, sketch_mat):
        ...
        if self.inmodel:
            ...
            for key in self.model.keys():
                loc = self.part[key]
                sketch_part = sketch_mat[loc[1]:loc[1] +
                                         loc[2], loc[0]:loc[0]+loc[2], :]
                ...
                self.shadow[key][loc[1]:loc[1] + loc[2], loc[0]:loc[0] +
